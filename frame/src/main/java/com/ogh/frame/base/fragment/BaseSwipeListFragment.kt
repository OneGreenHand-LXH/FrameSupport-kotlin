 ...
 */
abstract class BaseSwipeListFragment<T : ViewBinding, P : BasePresenter<*>, B : BaseBean, AB> : BaseSwipeFragment<T, P, B>(), BaseSwipeListView<B> {
    var mRecyclerView: RecyclerView? = null
    lateinit var mBaseAdapter: BaseQuickAdapter<AB, BaseQuickHolder>
    private var page: Int = BaseConfig.ViewPage.START_INDEX

    override fun initCommon() {
        super.initCommon()
        mRecyclerView = viewBinding.root.findViewById(R.id.frame_recycleView)
...
//      getLoadMoreModule().loadMoreView = CustomLoadMoreView()
    ...
    //自动更新adapter状态
    open fun notifyAdapterStatus(data: List<AB>?, pageIndex: Int, pageCount: Int) {
    ...
    /**
     * 设置适配器,若[userAdapterEmpty()]为true,切换布局只需setList(null)
     */
    open fun getAdapter() {
        if (userAdapterEmpty()) {
            val mEmptyView = LayoutInflater.from(mActivity).inflate(getEmptyView(), mRecyclerView!!.parent as ViewGroup, false)
